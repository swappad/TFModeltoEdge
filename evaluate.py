...
def graph_eval(input_graph_def, input_node, output_node):
    ...
    for index in range(0, calib_batch_size):
      ...
      img = cv2.imread(paths[(10 * calib_batch_size + index) % len(paths)], 1)
      try:
    ...
    with tf.Session() as sess:
        ...
        y = np.reshape(y_pred, (256, 512, 4))
        print(y_pred.shape)
        print(y.shape)
        print(y_pred.dtype)
        print(y.astype(np.uint32))
        image = result_map_to_img(y)
